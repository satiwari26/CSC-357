...
int main(int argc,char ** argv){
    ...
    UserRatio = (float)atof(argv[3]);

    FILE *imageFileLarge = fopen(argv[1], "rb");   //open the large file to read the content
    ...
    FILE *imageFileSmall = fopen(argv[2], "rb");   //open the smaller file to read the content
    ...
    FILE *aFile = fopen(argv[4], "wb");   //open the file to write the content
    ...
    if(infoheaderLarge.biSizeImage>infoheaderSmall.biSizeImage){    //if the first file size is bigger
        ...
        for(int x =0;x<infoheaderLarge.biWidth;x++){
            for(int y=0;y<infoheaderLarge.biHeight;y++){
                ...
                //interpolate
                unsigned char blue_left = blue[0] *(dy) + blue[2]*(1-dy);
                unsigned char blue_right = blue[1]*(dy) + blue[3]*(1-dy);
                ...
                //interpolate
                unsigned char green_left = green[0] *(dy) + green[2]*(1-dy);
                unsigned char green_right = green[1]*(dy) + green[3]*(1-dy);
                //storing blue in the highRez list
                ...
                //interpolate
                unsigned char red_left = red[0] *(dy) + red[2]*(1-dy);
                unsigned char red_right = red[1]*(dy) + red[3]*(1-dy);
                //storing blue in the highRez list
        ...
        for(int p =0;p<(real_width3*infoheaderLarge.biHeight);p++){
            valHighRez[p] = (1-UserRatio)*valLarge[p]+valHighRez[p]*(UserRatio);
        ...
        for(int p =0;p<(real_width3*infoheaderSmall.biHeight);p++){
            valHighRez[p] = (1-UserRatio)*valSmall[p]+valHighRez[p]*(UserRatio);
        fwrite(&valHighRez[p],sizeof(valHighRez[p]),1,aFile);
        }
