...
char grading(unsigned char value, float R_factor,float G_factor,float B_factor, int index, LONG real_width,LONG width){
    if(index%(real_width)>=(width*3)){
        return 0;
    }
    if(index % 3 == 0) { // blue
        value = value * B_factor;
    } else if(index % 3 == 1) { // green
        value = value * G_factor;
    } else { // red
        value = value * R_factor;
    }
...
int main(){
    ...
    val = (unsigned char *)mmap(NULL,(real_width*infoheader.biHeight*sizeof(unsigned char)),PROT_READ | PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS,-1,0);
    ...
    pid = fork();
        if(pid==0){  //child process
            for(int i =0;i<((halfHeight));i++){
                val[i]=grading(val[i],1,0,0,i,real_width,infoheader.biWidth);
                }
                return 0;
        }
        else if(pid>0){   //parent process
            for(int j =(halfHeight);j<(real_width*infoheader.biHeight);j++){
            val[j] = grading(val[j],0,1,0,j,real_width,infoheader.biWidth);
            }
        }
        else{
